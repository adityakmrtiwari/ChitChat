...
const ChatSidebar = ({
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent 
        side="right" 
        className="bg-black/90 backdrop-blur-2xl border-l border-blue-900/50 w-full max-w-full sm:w-80 md:w-96 p-0 shadow-2xl transition-all duration-300"
        style={{ maxWidth: '100vw' }}
      >
        {/* Attach swipe handlers only for mobile/modal */}
        <div {...swipeHandlers} className="h-full w-full">
          ...
          <div className="p-4 sm:p-6 space-y-6">
            ...
            <motion.div 
              ...
              className="space-y-3 max-h-[60vh] sm:max-h-96 overflow-y-auto scrollbar-hide"
            >
              <AnimatePresence>
                {filteredUsers.map((user, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="group flex items-center justify-between p-4 sm:p-5 bg-gradient-to-r from-blue-900/20 to-purple-900/20 rounded-2xl border border-blue-700/20 hover:bg-gradient-to-r hover:from-blue-900/30 hover:to-purple-900/30 transition-all duration-300 hover:border-blue-600/40 shadow-lg min-h-[56px] sm:min-h-[64px]"
                    >
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <Avatar className="h-10 w-10 sm:h-12 sm:w-12 ring-2 ring-blue-500/30 group-hover:ring-blue-400/50 transition-all duration-300 flex-shrink-0">
                        ...
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          ...
                          {isOwner && <Crown className="w-5 h-5 text-yellow-400 flex-shrink-0" />}
                          {isOnline ? (
                            <Wifi className="w-5 h-5 text-green-400 flex-shrink-0" />
                          ) : (
                            <WifiOff className="w-5 h-5 text-gray-500 flex-shrink-0" />
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {canRemoveUser(user._id) && !isCurrentUser && (
                          <Button
                            ...
                            className="text-red-400 hover:text-red-600 hover:bg-red-500/10 transition-all duration-200 h-12 w-12 sm:h-12 sm:w-12 p-0 min-w-[44px] min-h-[44px]"
